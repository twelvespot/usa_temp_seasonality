...
```{python}
county_data = (
    pl.read_excel("./data/county_lat_longs.xlsx")
...
if all_temp_data:
    ...
    print(counties_annual.head())
    print(counties_annual.describe())
else:
    print("No weather data was retrieved.")
```

Now since that took an hour or to to run, will save to parquet

```{python}
counties_annual.write_parquet("./data/annual_temps_county.parquet")
```

Will continue in 3_py_temp_calcs.qmd